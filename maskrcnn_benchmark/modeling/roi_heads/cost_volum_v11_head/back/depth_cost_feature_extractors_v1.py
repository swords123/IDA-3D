...
class DepthCostFeatureExtractor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        #------------------------------lalala----------------------
        self.avg_pool = nn.AvgPool2d(resolution,resolution)
        #self.avg_pool = nn.MaxPool2d(resolution,resolution)
        
        self.dres0 = nn.Sequential(convbn_3d(96, 32, 3, 1, 1),nn.ReLU(inplace=True),
                                     convbn_3d(32, 32 * 2, 3, (1,2,2), 1),nn.ReLU(inplace=True))
        
        self.max_pool1 = nn.MaxPool3d((1,2,2))

        self.dres1 = nn.Sequential(convbn_3d(32 * 2, 32 * 2, 3, 1, 1),nn.ReLU(inplace=True),
                                   convbn_3d(32 * 2, 32 * 2, 3, (1,2,2), 1)) 
    ...
    def depth_cost(self, cost, depth_bin, num_channels):
        ...
        cost_pool_1 = self.max_pool1(cost)

        cost = cost * x_cross
        
        cost = self.dres1(cost) + cost_pool_1
        cost = self.dres2(cost)

        cost_disp = torch.squeeze(cost, 1)
        cost_disp = self.avg_pool(cost_disp)
        cost_disp = cost_disp.squeeze()
