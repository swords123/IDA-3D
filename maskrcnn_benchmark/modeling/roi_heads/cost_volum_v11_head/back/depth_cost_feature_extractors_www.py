...
class DepthCostFeatureExtractor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.dres0 = nn.Sequential(convbn_3d(96, 32, 3, 1, 1),nn.ReLU(inplace=True),
                                     convbn_3d(32, 32, 3, 1, 1),nn.ReLU(inplace=True))

        self.max_pool1 = nn.MaxPool3d((1,2,2))

        self.dres1 = nn.Sequential(convbn_3d(32, 32 * 2, 3, 1, 1),nn.ReLU(inplace=True),
                                   convbn_3d(32 * 2, 32 * 2, 3, 1, 1)) 

        self.max_pool2 = nn.MaxPool3d((1,2,2))

        self.dres2 = nn.Sequential(convbn_3d(32 * 2, 16 * 2, 3, 1, 1),nn.ReLU(inplace=True),
                                    nn.Conv3d(16 * 2, 1, kernel_size=3, padding=1, stride=1,bias=False)) 
        '''
        ...
        self.dres0 = nn.Sequential(convbn_3d(96, 32, 3, 1, 1),nn.ReLU(inplace=True),
                                     convbn_3d(32, 32, 3, 1, 1),nn.ReLU(inplace=True))
        self.max_pool0 = nn.MaxPool3d((1,2,2))

        self.dres1 = nn.Sequential(convbn_3d(32, 32*2, 3, 1, 1),nn.ReLU(inplace=True),
                                   convbn_3d(32*2, 32*2, 3, 1, 1)) 
        self.max_pool1 = nn.MaxPool3d((1,2,2))

        self.dres2 = nn.Sequential(convbn_3d(32*2, 32*4, 3, 1, 1),nn.ReLU(inplace=True),
                                   convbn_3d(32*4, 32*4, 3, 1, 1))
        self.max_pool2 = nn.MaxPool3d((1,2,2)) 
                                   
        #self.cls1 = nn.Sequential(convbn_3d(32*2, 16*2, 3, 1, 1),nn.ReLU(inplace=True),
        #                            nn.Conv3d(16*2, 1, kernel_size=3, padding=1, stride=1,bias=False))

        self.cls2 = nn.Sequential(convbn_3d(32*4, 16*2, 3, 1, 1),nn.ReLU(inplace=True),
                                    nn.Conv3d(16*2, 1, kernel_size=3, padding=1, stride=1,bias=False))  
    ...
    def depth_cost(self, cost, depth_bin, num_channels):
        ...
        cost0 = self.max_pool0(cost0)
        cost0 = cost0 * x_cross
        
        cost1 = self.dres1(cost0)
        cost1 = self.max_pool1(cost1)

        cost2 = self.dres2(cost1)
        cost2 = self.max_pool2(cost2)

        #cost_cls1 = self.cls1(cost1)
        cost_cls2 = self.cls2(cost2)



        #------2-------
        cost_disp2 = torch.squeeze(cost_cls2, 1)
        cost_disp2 = self.avg_pool_2(cost_disp2)
        cost_disp2 = cost_disp2.squeeze()
        disp_prob2 = F.softmax(cost_disp2,-1)
        disp2 = Variable(torch.FloatTensor(disp_prob2.size()[0]).zero_()).cuda()
        for i in range(self.max_depth):
            disp2 += disp_prob2[:,i] * depth_bin[:,i]
        disp2 = disp2.contiguous()
